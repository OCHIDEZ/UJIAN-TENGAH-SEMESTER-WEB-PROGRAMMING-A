...
function loadDataPenulis() {
        ...
        if (fileFoto) {
            ...
            const ukuranFile = fileFoto.size / 1024 / 1024;
...
function editPenulis(id) {
    ...
    .then(res => res.json())
    .then(data => {
        document.getElementById('modalTitlePenulis').innerText = 'Edit Penulis';
        document.getElementById('btnSubmitPenulis').innerText = 'Simpan Perubahan';
...
function loadDataArtikel() {
    ...
    .then(data => {
        ...
        if(data.length === 0) {
            ...
            data.forEach(item => {
            let ext = "NONE";
            let namaFile = item.gambar_url || item.gambar || "";

            if (namaFile && namaFile.includes('.')) {
                ext = namaFile.split('.').pop().toUpperCase();
            }

                let kotakGambar = `<div style="width: 45px; height: 45px; background-color: #f4f5f7; border-radius: 6px; display: flex; align-items: center; justify-content: center; color: #b3b8c2; font-weight: bold; font-size: 11px; border: 1px solid #eaedf1;">${ext}</div>`;

                html += `
                    <tr>
                        <td>${kotakGambar}</td> <td><b>${item.judul}</b></td>
                        <td><span class="badge">${item.nama_kategori}</span></td>
                        <td>${item.nama_depan}</td>
                        <td style="font-size:11px; color:#888;">${item.hari_tanggal}</td>
                        <td>
...
function editArtikel(id) {
    ...
    .then(data => {
        ...
        fetch('ambil_penulis.php').then(res => res.json()).then(penulis => {
            let opt = '<option value="">Pilih Penulis</option>';
            penulis.forEach(p => opt += `<option value="${p.id}">${p.nama_depan} ${p.nama_belakang}</option>`);
            document.getElementById('opt_penulis').innerHTML = opt;
            document.getElementById('opt_penulis').value = data.id_penulis;
        });

        fetch('ambil_kategori.php').then(res => res.json()).then(kategori => {
            let opt = '<option value="">Pilih Kategori</option>';
            kategori.forEach(k => opt += `<option value="${k.id}">${k.nama_kategori}</option>`);
            document.getElementById('opt_kategori').innerHTML = opt;
...
<textarea name="isi" id="isi_artikel" required style="width:96%; height:150px; padding:8px;"></textarea></p>
            <p>Gambar Artikel<br>
            <input type="file" name="gambar" accept="image/*" style="width:96%; padding:8px;">
            <br><small style="color: gray;">*Kosongkan jika tidak ingin menambahkan atau mengganti gambar.</small>
        </p>
